...
RegisterNetEvent('fivem_cardealer:ReplaceVehSV')
AddEventHandler('fivem_cardealer:ReplaceVehSV', function(model, name, currentID)
	MySQL.Async.execute("UPDATE vehicle_display SET model=@model,name=@name,commission=@commission WHERE id=@id",{['@model'] = model, ['@name'] = name, ['@commission'] = '10', ['@id'] = currentID},function(...)
		TriggerClientEvent('fivem_cardealer:ReplaceVehCL', -1, model, currentID)
	end)
end)
