...
AddEventHandler('esx:addInventoryItem', function(item, count)
    ...
    if ESX.UI.Menu.IsOpen('default', 'tprp_base', 'inventory') then
        ESX.ShowInventory()
    end
end)
...
AddEventHandler('esx:pickup', function(id, label, player)
    ...
    ESX.Game.SpawnLocalObject('prop_michael_backpack', {
        x = x,
        y = y,
        z = z - 2.0,
    }, function(obj)
...
AddEventHandler('esx:pickup', function(id, label, player)
    ...
    local vehicle = GetVehiclePedIsIn(ped,false)
        ...
        if GetEntityHeading(ped) <= 360 and GetEntityHeading(ped) >=270 then
        ...
        elseif GetEntityHeading(ped) <= 269 and GetEntityHeading(ped) >=180 then
            print(GetEntityHeading(ped))
            y = y-3
        ESX.Game.SpawnLocalObject('prop_michael_backpack', {
            x = x,
        ...
        elseif GetEntityHeading(ped) <= 179 and GetEntityHeading(ped) >=90 then
            print(GetEntityHeading(ped))
            x = x+3.0
        ESX.Game.SpawnLocalObject('prop_michael_backpack', {
            x = x,
        ...
        elseif GetEntityHeading(ped) <= 89 and GetEntityHeading(ped) >=0 then
            print(GetEntityHeading(ped))
            y = y+3.0
        ESX.Game.SpawnLocalObject('prop_michael_backpack', {
            x = x,
